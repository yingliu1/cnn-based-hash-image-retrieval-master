...
from train_resnet50 import softmax_pretrain_on_dataset
...
def pair_tune(source_model_path, train_generator, val_generator, tune_dataset, batch_size, num_classes):
    ...
    model.save('./model/' + tune_dataset + '_pair_model.h5')


def pair_pretrain_on_dataset(source, project_path):
    if source == 'market':
        train_list = project_path + '/dataset/dataset-list/market_train.list'
        train_dir = project_path + '/dataset/Market-1501/train'
        class_count = 751
    elif source == 'cuhk01':
        train_list = project_path + '/dataset/dataset-list/cuhk01.list'
        ...
        train_list = project_path + '/dataset/dataset-list/mix.list'
...
if __name__ == '__main__':
    ...
    # sys.path.extend([path1])

    # sources = ['cuhk01', 'cuhk03', 'market', 'mix', 'mix2']
    sources = ['mix2']
    for source in sources:
        softmax_pretrain_on_dataset(source, path1)
        pair_pretrain_on_dataset(source, path1)